...
shinyServer (function(input, output) {
  ...
  output$Harbindescribe <- renderUI({
    ...
    if(is.null(answer))
      return(NULL)
    if(answer == 1){
      h5(strong("Choose between the Kolmogorov-Smirnov test or the Harbin test to compare normalised data to a reference database."), em("Kolmogorov-Smirnov test is the default. The harbin test is more conservative and has a higher false negative rate. The harbin test is also applicable if you want your database to have more weight than your test data in the comparison test."))
    }
  })
  ...
  GOI_o <- function(){
    ...
    for(j in 1:nfiles){
      ...
      varnames<-scan(file,what="character",nlines=1,skip=8,sep=",",quiet=TRUE) # scan variable names on 9th line
      tempdata<-scan(file,what="character",skip=9,sep=",",quiet=TRUE) # scan all data and store in vector
